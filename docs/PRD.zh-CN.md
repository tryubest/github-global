...
- **US-06 提交 PR**
  As an 开源作者，I want 翻译完成后产物以 PR 形式提交到我的仓库，So that 我能 review 后再合并，保障主干质量。
- **US-07 保护代码**
  As an 开源作者，I want 翻译器自动跳过代码块、行内代码、URL、图片链接，So that 技术内容不会被 AI 改坏。
...
- **US-08 增量翻译**
  As an 开源作者，I want 仓库每次 push 后，只有变化过的 md 文件被重新翻译，So that 节省时间和 token 成本。
- **US-09 术语表**
